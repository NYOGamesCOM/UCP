...
if($data->playerGroupRank == 1)
	return print_r(json_encode(array('message' => 'Player <i>'.$data->playerName.'</i> has minimum rank (1).','color' => 'red')));
...
if($qa)
	return print_r(json_encode(array('message' => 'Player <i>'.$data->playerName.'</i> is now '.$ranks->$rank.'.','color'=>'green','rank'=>$ranks->$rank)));
else
	return print_r(json_encode(array('message' => 'An error occured.','color' => 'red')));
